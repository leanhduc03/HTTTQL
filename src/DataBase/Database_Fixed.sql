...
alter table HoaDon add (
    constraint HD_NgayHD_NNULL check (TenMon is not null),
...
--Tim Code Voucher, Loai mon an duoc Ap dung Voucher tu bang Voucher
    ...
    IF(v_code IS NOT NULL) THEN
        IF(v_LoaiMA='All' OR v_LoaiMA=MA_Loai) THEN 
            IF INSERTING THEN    
                UPDATE HoaDon SET TienGiam = TienGiam + CTHD_Tinhtiengiam(:new.ThanhTien,v_code) WHERE HoaDon.ID_HoaDon=:new.ID_HoaDon;
            END IF;
            
            IF UPDATING THEN    
                UPDATE HoaDon SET TienGiam = TienGiam + CTHD_Tinhtiengiam(:new.ThanhTien,v_code) - CTHD_Tinhtiengiam(:old.ThanhTien,v_code) WHERE HoaDon.ID_HoaDon=:new.ID_HoaDon;
            END IF;
            
            IF DELETING THEN    
                UPDATE HoaDon SET TienGiam = TienGiam - CTHD_Tinhtiengiam(:old.ThanhTien,v_code) WHERE HoaDon.ID_HoaDon=:old.ID_HoaDon;
            END IF;
        END IF;
    END IF;
END;
...
CREATE OR REPLACE FUNCTION CTHD_CTHD_Tinhtiengiam(Tongtien Number,Code Voucher.Code_Voucher%TYPE)
RETURN NUMBER
IS 
    Tiengiam NUMBER;
    v_phantram NUMBER;
